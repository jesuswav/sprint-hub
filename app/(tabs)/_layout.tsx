...
export default function TabLayout() {
  ...
  function HeaderTitle() {
    ...
    return (
      <View style={styles.headerTitleContainer}>
        <Ionicons name="rocket" size={20} color={Colors[colorScheme ?? 'light'].text} />
        <Text style={[styles.headerTitleText, { color: Colors[colorScheme ?? 'light'].text }]}>
          Mi Aplicación
        </Text>
      </View>
    );
  ...
  function CustomDrawerItem({ label, iconName, onPress }: CustomDrawerItemProps) {
    const colorScheme = useColorScheme();
  
    return (
      <TouchableOpacity onPress={onPress} style={styles.drawerItemContainer}>
        <Ionicons name={iconName} size={24} color={Colors[colorScheme ?? 'light'].text} />
        <Text style={[styles.drawerItemText, { color: Colors[colorScheme ?? 'light'].text }]}>
          {label}
        </Text>
      </TouchableOpacity>
    );
    ...
    <Drawer
      screenOptions={{
        ...
        headerRight: () => (
          ...
          >
            <Ionicons name="menu" size={48} color={Colors[colorScheme ?? 'light'].text} />
          </TouchableOpacity>
        ),
